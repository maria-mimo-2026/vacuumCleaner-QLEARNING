...
"""
QL with an aguemented state: position, dirt, visit_level.
where visit level is the number of visits to the current 
room (position) capped intuitively by some number VISITS_CAP
...
from vacuum.policy.helpers import plot_epsilon, plot_visit_heatmap
...
# should dependent on the map size and complexity?! (check epsilon decay function)
# a prompt to change train episodes is given in main.py
...
# cap to calculate 'visit_level' (part of the agent' state)
VISITS_CAP = 7
"""
In a minimal environment with plus shape (map_id: vacuum-5rooms-v1), 
which contains 5 rooms, one in the middle, one on each side,
the agent would need to revisit the middle room four times, each time 
going to a different direction in order to visit all room. 
Then, a minimal visit level cap should be 5.
"""

from .registry import register_policy

@register_policy("q-learning_1")
class QLearnPolicy(CleanPolicy):

    def __init__(self, world_id, env):
        super().__init__("q-learning_1", world_id, env)
        self.trained = False
        self.q_table = None
    ...
    def train_q_learning(self, env, episodes=TRAIN_EPISODES, learning_rate_a=LEARNING_RATE_ALPHA, 
        ...
        # init empty QL table
        self.q_table = np.zeros((num_states, env.action_space.n), dtype=np.float32)  
        # performance stats over episodes
        dirty_rooms = np.zeros(episodes, dtype=np.int16)        # nbr of dirty rooms initially
        rewards = np.zeros(episodes)
        cleanings = np.zeros(episodes, dtype=np.float32)
        travels = np.zeros(episodes)
        epsilon_log = []
        # @DEBUG: show simulation
        ...
        plot_epsilon(episodes, epsilon_log, EPSILON_DECAY_RATE, policy_name=self.policy_id,
            map_name=self.world_id, save=True)    
        ...
        print(" [DEBUG] ")
        print(f"initial number of dirty rooms per episode: {dirty_rooms}")
        print(f" Q-table shape: ")
        tl.print_matrix(self.q_table, "Q-table (final)")
        ...
        if input("[prompt] Save training results? [y/n]") == 'y':
            ...
            tl.save_training_results(self.world_id, self.policy_id, results)
